...
The Drive service MUST expose an `uploadFile(buffer, originalName, mimeType)` method that uploads a file to the configured Drive folder and returns `{ driveFileId, webViewLink }`.
...
- AND the method returns `{ driveFileId: string, webViewLink: string | null }`

#### Scenario: Upload fails with Drive API error

- GIVEN the Drive API returns an error (e.g., quota exceeded, permission denied)
- WHEN `uploadFile()` is called
- THEN the method throws an error with the Drive API error details
