...
4. **B7 FIX:** The dialog MUST NOT close automatically if image upload fails. The user MUST see the error and have the option to retry or close manually. If image upload succeeds, the dialog MUST close and the list MUST show the new product with the image without requiring a manual refresh.
