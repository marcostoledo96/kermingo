 ...
 * Lista pedidos operativos para cocina (excluye cancelado).
...
function transicionCocinaValida(actual, siguiente) {
  if (actual === siguiente) return false;
  return (TRANSICIONES_COCINA[actual] || []).includes(siguiente);
}
...
export async function cambiarEstadoCocina(req, res, next) {
  try {
    ...
    if (!transicionCocinaValida(actual, siguiente)) {
    ...
    if (result === -1) {
      throw new ValidationError('Transición de estado no válida');
    }
